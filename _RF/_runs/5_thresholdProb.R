...
layer <- raster(paste0(path, '/_RF/', run, '/_results/_raw/_current/RF_5Prob_current.asc'))

occ <- read.csv(paste0(path, '/_points/_csv/4_occ_swd_rm.csv')) %>%
...
values_df       <- as.data.frame(quantiles_1)
...
# threshold       <- as.numeric(subset(values_df, rownames(values_df) == '1%'))#threshold <- min(value)
save(threshold, file = paste0(path, '/_rData/', run, '/threshold_prob.rData'))
