...
shp <- shapefile('_shp/_base/_admCountries/CAM_Adm0.shp') #dplyr::select(shp@data, NAME_0, NAME_ENGLI)
...
dup <- dup_cell(path_mask = '_raster/_mask/mask_cam_2_5min.tif', path_df = '_points/_csv/6_occ_swd.csv')
occ <- dup[[2]]
...
write.csv(occ, '_points/_csv/6_occ_swd_rm.csv', row.names = F)
...
# Random Forest Clustering - Presences
