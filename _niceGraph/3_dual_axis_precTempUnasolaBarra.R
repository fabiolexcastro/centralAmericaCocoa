...
source('../_code/_r/dual_axis_function.R')
...
gcms_30 <- filter(all_data2, Period == '2030') %>% tbl_df()
...
gg <- gg + ggtitle("Polygons Mandalay") + theme(panel.grid.major = element_line())#theme(panel.grid.major = element_line(colour = "grey"))
...
png(filename = 'presen2.png', width = 9, height = 8, units = 'in', res = 300)
...
#panel.grid.major.y = element_blank(),
#panel.grid.minor.y = element_blank())
gg <- gg + ggtitle("Polygons Kiaukme") + theme(panel.grid.major = element_line(colour = "grey"))
